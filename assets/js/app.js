...
import { initCrypto } from './crypto.js?v=65';
import { loadConfig, getApp, getUsers, getSession, login, logout } from './auth.js?v=65';
import { h, clear, toast, initials, brandStrip } from './ui/components.js?v=65';
import { renderLogin } from './ui/login.js?v=65';
import { renderMatches, renderHistory, resetPosIndex } from './ui/matches.js?v=65';
import { renderTable } from './ui/table.js?v=65';
import { renderRules } from './ui/rules.js?v=65';
import { maybeOnboard } from './ui/onboarding.js?v=65';
import { setupPullToRefresh } from './ui/pull-refresh.js?v=65';
import { setupDrawerSwipe } from './ui/drawer-swipe.js?v=65';
import { pushSupported, pushState, enablePush, disablePush, registerSW, refreshSubscription, isIOS, isStandalone } from './push.js?v=65';
import { maybeShowInstallPrompt, showInstallPrompt } from './ui/install-prompt.js?v=65';
...
export async function loadPublicData() {
  const [matches, squads, players, standings, fifa, favTeams, favScorers, scorerPos] = await Promise.all([
  ...
  S.scorerPos = scorerPos || {};
  resetPosIndex(); // данные обновились — пересоберём индекс позиций (с учётом заморозки)
}
...
function buildShell() {
  const sidebar = h('aside', { class: 'sidebar', id: 'sidebar' }, [
    h('a', { class: 'brand', href: '#matches', 'aria-label': 'На главную', onclick: (e) => { e.preventDefault(); navigate('matches'); } }, [
      h('img', { class: 'brand-logo', src: 'assets/img/logo.png?v=65', alt: 'ЛВС', width: 52, height: 52 }),
...
async function startApp() {
  ...
  if (pushSupported()) { registerSW(); refreshSubscription(S.session, S.app); } // воркер + обновляем подписку при ротации iOS
